...
import { pinOwner, rankOf, type GroupAdmin } from "../../lib/nostr/permissions.ts";
...
export function MemberList({ members, onDm }: Props) {
  ...
  const roster = useGroupStore((state) => state.admins) ?? [];
  const square = useGroupStore((state) => state.active());
  const owner = useGroupStore((state) => state.isOwner());
  const admins = pinOwner(roster, square?.owner ?? (owner && me ? me : undefined));
