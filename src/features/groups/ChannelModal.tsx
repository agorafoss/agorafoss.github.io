...
export function ChannelModal({ onClose, defaultKind = "text" }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    const secret = await addChannel(name, kind, locked);
        ...
        {reveal ? (
          ...
          <>
            ...
            <p className={styles.lead}>
              {kind === "voice" ? t("channels.addLeadVoice") : t("channels.addLead")}
            </p>
            <form className={styles.form} onSubmit={(event) => void submit(event)}>
              ...
              {locked ? (
                <p className={styles.lead}>{kind === "voice" ? t("channels.privateLead") : t("channels.privateTextLead")}</p>
              ) : null}
